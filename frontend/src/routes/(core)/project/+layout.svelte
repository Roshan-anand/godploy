<script lang="ts">
	...
	const createServiceMutation = createMutation(() => ({
		...
		onSuccess: async (createdService) => {
			...
			// goto(resolve(`/services/${createdService.type}?id=${createdService.id}`));
			goto(
	...
	<Dialog.Portal>
		...
		>
			...
			<Dialog.Description class="text-sm text-muted-foreground">
				Create a new service for your project.
			</Dialog.Description>
			...
			>
				...
				<form.Field
					name="name"
					validators={{
						onChange: z.string().min(3, 'Service name must be at least 3 characters')
					}}
				>
					{#snippet children(field)}
						<div class="space-y-1.5">
				...
				<form.Field
					name="app_name"
					validators={{
						onChange: z.string().min(3, 'App name must be at least 3 characters')
					}}
				>
					{#snippet children(field)}
						<div class="space-y-1.5">
